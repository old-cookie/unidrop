...
class WebVideoEditorPage extends StatefulWidget {
  const WebVideoEditorPage({
    super.key,
  ...
  @override
  State<WebVideoEditorPage> createState() => _WebVideoEditorPageState();
}

class _WebVideoEditorPageState extends State<WebVideoEditorPage> {
  ...
  Future<void> _initializeEditor() async {
    try {
      final video = EditorVideo.memory(widget.videoBytes);
      ...
      final controller = VideoPlayerController.networkUrl(dataUri);
      await Future.wait([
  ...
  Future<void> _generateThumbnails(EditorVideo video) async {
    if (_videoMetadata == null || !mounted) return;
  ...
  Future<void> _generateVideo(CompleteParameters parameters) async {
    ...
    try {
      ...
      final data = VideoRenderData(
        id: _taskId,
        video: EditorVideo.memory(widget.videoBytes),
      ...
      _outputBytes = await _proVideoEditor.renderVideo(data);
    } catch (e) {
      _outputBytes = null;
  ...
  Future<void> _handleCloseEditor(EditorMode editorMode) async {
    ...
    Navigator.pop(context, _outputBytes);
  }
